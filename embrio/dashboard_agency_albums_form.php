<?php
	if($_POST["album_name"]){
		$db->addtable("agency_albums");	$db->where("album_id",$album_id);
	...
	foreach($_FILES["album"]["tmp_name"] as $xx => $tmp_name){
		if($tmp_name){
			...
			$agency_albums_id = $db->fetch_single_data("agency_albums","id",["user_id" => $__user_id,"album_id" => $album_id,"seqno"=>$xx]);
			$agency_albums_filename = $db->fetch_single_data("agency_albums","filename",["user_id" => $__user_id,"album_id" => $album_id,"seqno"=>$xx]);
			$db->addtable("agency_albums");
			$db->addfield("filename");	$db->addvalue($albumFilename);
			$db->addfield("status");	$db->addvalue(0);
			$db->addfield("album_name");$db->addvalue($_POST["album_name"]);
			if($agency_albums_id > 0){
	...
	<form id="frmAlbum" action="?tabActive=album&album_id=<?=$album_id;?>&editing=1" method="POST" enctype="multipart/form-data">
		...
		<?php for($xx=0;$xx<$rowAlbum;$xx++){ ?>
			<?php
				$albumFilename = $db->fetch_single_data("agency_albums","filename",["user_id"=>$__user_id,"album_id"=>$album_id,"seqno"=>$xx]);
